...
def dlvid(o, v, d):
    if o == 'P':
        ...
        if o == 'D':
            ...
            viddir = '{}{}.mp4'.format(dldir, v.title)
            video = ffmpeg.input('{}{}.mp4'.format(dldir, v.title))
            auddir = '{}{}.webm'.format(dldir, v.title)
            audio = ffmpeg.input(auddir)
            out = ffmpeg.concat(video, audio, v=1, a=1).output('{} {}.mp4'.format(dldir, v.title)).run()
            os.remove(viddir)
            os.remove(auddir)
        return True
    else:
